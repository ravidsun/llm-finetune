#!/bin/bash
# Unified RunPod Script - Setup and Train LLM in One Command
# Usage:
#   bash runpod.sh setup    # First-time setup
#   bash runpod.sh train    # Start training
#   bash runpod.sh all      # Setup + Train (default)
...
setup() {
    ...
    if command -v nvidia-smi &> /dev/null; then
        ...
        echo -e "${RED}❌ No GPU detected! This script requires a GPU.${NC}"
        exit 1
    fi
    ...
    cd /workspace || { echo -e "${RED}❌ Failed to access /workspace${NC}"; exit 1; }
    ...
    echo ""
    echo -e "${BLUE}[5/7] Installing dependencies (3-5 minutes)...${NC}"
    pip install -q -e .

    # Step 6: Create directory structure
    echo ""
    echo -e "${BLUE}[6/7] Creating directory structure...${NC}"
    mkdir -p data/input processed_data output configs
    ...
    echo "      bash scripts/runpod.sh train"
    echo ""
}
...
train() {
    echo "=================================================="
    echo "🚀 RunPod Quick Train - Starting Training"
    echo "=================================================="
    echo ""

    # Change to project directory
    cd /workspace/llm-finetune || { echo -e "${RED}❌ Not in RunPod environment${NC}"; exit 1; }
    ...
    if [ "$JSONL_COUNT" -eq 0 ]; then
        ...
        echo "  1. Use RunPod UI: Connect → Upload Files → /workspace/llm-finetune/data/input/"
...
SCRIPT_EOF
    ...
    if [ "$TOTAL_LINES" -lt 1000 ]; then
        echo "Estimated training time: ~30-60 minutes (small dataset)"
    elif [ "$TOTAL_LINES" -lt 5000 ]; then
        echo "Estimated training time: ~1-3 hours (medium dataset)"
    else
        echo "Estimated training time: ~3-8 hours (large dataset)"
    fi
    echo ""
}
...
case "$COMMAND" in
    ...
    all)
        ...
        ;;
    *)
        echo "Usage: bash runpod.sh [setup|train|all]"
        echo ""
        echo "Commands:"
        echo "  setup  - Set up environment (first time)"
